...
class MenopauseCognitionAnalysis:
    ...
    def create_stratified_forest_plot(self, outcome='TOTIDE1'):
        ...
        for strat_name, strat_var in stratifications.items():
            if strat_var is None:
                ...
                key = f"{outcome}"
                if key in self.mixed_model_results:
                    result = self.mixed_model_results[key]
                    ...
                    for symptom in symptoms_to_plot:
                        if symptom in result.params.index:
                            ...
                            plot_data.append({
                                ...
                                'Symptom': self.var_labels.get(symptom, symptom),
                                'Coefficient': coef,
                                'SE': se,
                ...
                # Stratified analysis
                if strat_var not in self.data.columns:
                    print(f"Warning: {strat_var} not in data, skipping {strat_name}")
                    continue

                # Get unique categories
                categories = self.data[strat_var].dropna().unique()

                for category in categories:
                    # Subset data and reset index
                    subset_data = self.data[self.data[strat_var] == category].copy()
        ...
        fig.suptitle(f'Symptom Effects on {self.var_labels.get(outcome, outcome)}\nby Key Demographics',
        ...
        # Save
        output_file = os.path.join(self.output_dir, f'{outcome}_stratified_forest_plot.png')
    ...
    def run_complete_analysis(self):
        ...
        try:
            ...
            self.create_stratified_forest_plot(outcome='TOTIDE1')
