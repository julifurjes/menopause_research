...
class MenopauseCognitionMixedModels:
    ...
    def preprocess_data(self):
        ...
        self.data = self.data[relevant_vars + ['STATUS_Label', 'INCOME_original', 'DEGREE_original', 'AGE_BASELINE']]
    ...
    def transform_variables(self):
        ...
        log_transform_vars = ['NERVES', 'NUMHOTF', 'NUMNITS', 'NUMCLDS', 'FEARFULA']

        # Variables needing sqrt transformation (moderate positive skew 1.0-3.0)
        sqrt_transform_vars = ['SAD', 'IRRITAB', 'MOODCHG',
        ...
        no_transform_vars = ['STIFF', 'HELPSIC', 'STATUS', 'INCOME', 'DEGREE', 'AGE_BASELINE']
    ...
    def fit_mixed_models(self):
        """Fit linear mixed-effects models for cognitive function outcome only"""
        results = {}

        # Define the outcome to model (only cognitive function)
        outcomes = {
        ...
        for outcome_var, outcome_name in outcomes.items():
            ...
            try:
                ...
                # Store coefficients with standardized naming for visualization methods (cognitive_function only)
    ...
    def create_path_coefficient_forest(self, results):
        """Create forest plot for cognitive function outcome"""
        # Create a single plot for cognitive function only
        ...
        # Define outcome (only cognitive function)
        outcome = 'cognitive_function'
        outcome_name = 'Cognitive Function'
        ...
        if outcome not in results:
            print(f"No results found for {outcome_name}")
            return
        ...
        ax.set_title(f'Effects on {outcome_name}', fontsize=18, fontweight='bold')
